...
for f in os.listdir():
   ...
   #print(f_name)
   f_title, f_num = f_name.split('-')                               ##Now we can even further split this new variable to serve our ways
   #print(f_title)
   #print(f_num)
   ##Lastly we want to strip off the unnecessary empty spaces
   f_title = f_title.strip()
   f_num = f_num.strip()[1:].zfill(2)                              #'zfill' method allows us to create 0 padded strings and help avoid possible conflicts while and we strip the '#' from the beginning
   f_ext = f_ext.strip()
