...
print(f.read())                            #Will throw a 'ValueError', we can only work with this file within context manager
#-------------------------------------------------------------------------------------
...
with open('test.txt', 'r') as f:
    ...
    f_contents = f.readline()               #returns the first line of the file, and so on if called multiple times
...
with open('test.txt', 'r') as f:
    ...
    f.seek(0)                                           #We used 'seek(0)' to start at the beginning of the file, but the position can be set to anywhere we like
    f_contents = f.read(character_size)
    print(f_contents)
#-------------------------------------------------------------------------------------
## Writing files:
with open('test2.txt', 'w') as f:                        #This will go ahead and create a file if it does not exist, and overwrite a file if it does exist
    f.write('Test')                                      #Even if we used the 'pass' command, the file will be created because of the code on the line above
    f.seek(0)                                            #Using the 'seek' method with writing tends to complicate things
    f.write('R')                                         #This will return the string of 'Rest'; writes 'Test' returns to 0 index with 'seek' after only changes 'T' into a 'R'
#-------------------------------------------------------------------------------------
## Read & Write
